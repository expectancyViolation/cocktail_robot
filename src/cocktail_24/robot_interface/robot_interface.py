...
class RoboTcpInterface(Protocol):
    ...
    @staticmethod
    def gen_write_relays(
        relays: RobotRelays, data: bytes
    ...
    @staticmethod
    def gen_start_program(
        job_name: str,
    ) -> Generator[str, str, RoboTcpCommandResult]: ...

    @staticmethod
    def gen_set_job(
        job_name: str, line_number: int
    ) -> Generator[str, str, RoboTcpCommandResult]: ...
    ...
    @staticmethod
    def gen_write_var(
        v: RoboVarType, index: int, val: Any
...
class RoboTcpCommands(RoboTcpInterface):
    ...
    @staticmethod
    def _gen_hostctrl_(
        command, args: str | None = None
    ) -> Generator[str, str, str | None]:
    ...
    def _check_0000_(resp: str) -> RoboTcpCommandResult:
        return (
            RoboTcpCommandResult.ok if (resp == "0000") else RoboTcpCommandResult.error
        )
    ...
    def gen_write_relays(
        ...
        resp = yield from RoboTcpCommands._gen_hostctrl_(
            RoboTcpCommands.WRITE_CMD, arg_string
        )
    ...
    def gen_read_relays(relays: RobotRelays) -> Generator[str, str, bytes]:
        ...
        resp = yield from RoboTcpCommands._gen_hostctrl_(
            RoboTcpCommands.READ_CMD, arg_string
        )
    ...
    def gen_hold_on(on: bool) -> Generator[str, str, RoboTcpCommandResult]:
        resp = yield from RoboTcpCommands._gen_hostctrl_(
            RoboTcpCommands.HOLD, "1" if on else "0"
        )
        return RoboTcpCommands._check_0000_(resp)
    ...
    def gen_start_program(job_name: str) -> Generator[str, str, RoboTcpCommandResult]:
        resp = yield from RoboTcpCommands._gen_hostctrl_(
            RoboTcpCommands.START_CMD, job_name
        )
        return RoboTcpCommands._check_0000_(resp)
    ...
    def gen_read_status() -> Generator[str, str, RoboStatus | None]:
        ...
        safety_resp = yield from RoboTcpCommands.gen_read_relays(
            RobotRelays(address=80020, num_bytes=1)
        )
        ...
        parsed_status = RoboStatus.from_nums(
            num_1, num_2, safety > 0, job_pos=job_pos, success_count=int(success_count)
        )
    ...
    def gen_read_var(v: RoboVarType, index: int) -> Generator[str, str, Any]:
        resp = yield from RoboTcpCommands._gen_hostctrl_(
            RoboTcpCommands.READ_VAR, f"{v.value},{index}"
        )
    ...
    @staticmethod
    def gen_write_var(
        v: RoboVarType, index: int, val: Any, confirm: bool = True
    ) -> Generator[str, str, Any]:
        resp = yield from RoboTcpCommands._gen_hostctrl_(
            RoboTcpCommands.WRITE_VAR, f"{v.value},{index},{val}"
        )
        if resp != "0000":
    ...
    def gen_set_job(
        ...
        resp = yield from RoboTcpCommands._gen_hostctrl_(
            RoboTcpCommands.SET_JOB, f"{job_name},{line_number}"
        )
